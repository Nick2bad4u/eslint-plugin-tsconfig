 ...
 * }}
 *
 * @throws {Error} When an option is malformed or missing its value.
 */
 ...
 * @returns {string}
 *
 * @throws {Error} When no non-empty value is available.
 */
 ...
 *     | undefined}
 *
 * @throws {Error} When a `&lt;loc&gt;` tag is not closed properly.
 */
 ...
 *
 * @throws {Error} When the provided JSON is not an array of non-empty strings.
 */
 ...
 * @returns {string}
 *
 * @throws {Error} When the key format is invalid.
 */
...
/**
 * Parse and deduplicate all `&lt;loc&gt;` entries from a sitemap. Docusaurus
 * emits a standard XML sitemap with raw URL text content inside `&lt;loc&gt;`
 * elements. We only need those URL values and deliberately keep this parser
 * constrained to the sitemap contract rather than introducing a heavier XML
 * dependency for one deterministic extraction step.
 *
 * @param {string} sitemapXml
 *
 * @returns {readonly string[]}
 *
 * @throws {Error} When the sitemap does not contain any `&lt;loc&gt;` entries.
 */
